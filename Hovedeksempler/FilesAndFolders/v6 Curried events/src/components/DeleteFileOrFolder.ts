...
export class DeleteFileOrFolder extends BaseComponent {
    ...
    render() {
        ...
        if (isInInitialPhase) {
            ...
            btn.addEventListener('click', this.createChangePhaseHandler(false));
        } else {
            const btns = this.shadowRoot!.querySelectorAll('button')!;
            btns[0].addEventListener('click', this.handleDelete.bind(this));
            btns[1].addEventListener('click', this.createChangePhaseHandler(true));
        }
    }
    ...
    createChangePhaseHandler(isInInitialPhase: boolean) {
        return () => {
