...
import type { Params, MenuCategoryParams, MenuItemParams, EventOf } from "../types";
...
export class RouterView extends BaseComponent {
  ...
  private registerBusHandlers() {
    eventBus.on('ADDED_CATEGORY', (e: EventOf<'ADDED_CATEGORY'>) => {
      appModel.addCategory(e.category);
      const param = { category: e.category };
      eventBus.emit({ type: 'NAVIGATED', route: { name: 'menu', param } });
      //this.scheduleRender();
    });

    eventBus.on('SELECTED_CATEGORY', (e: EventOf<'SELECTED_CATEGORY'>) => {
    });
    eventBus.on('SELECTED_MENU_ITEM', (e: EventOf<'SELECTED_MENU_ITEM'>) => {
    });
    eventBus.on('ADDED_MENU_ITEM', (e: EventOf<'ADDED_MENU_ITEM'>) => {
    });
    eventBus.on('DELETED', (e: EventOf<'DELETED'>) => {
    });
  }
